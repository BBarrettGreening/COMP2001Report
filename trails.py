...
def read_all_admin():
    try:
        trails = db.session.execute(text("EXEC CW2.ReadTrailAdministrator")).fetchall()
...
def read_all_standard():
    try:
        trails = db.session.execute(text("EXEC CW2.ReadTrailStandard")).fetchall()
        return jsonify({"trails": trails_schema.dump(trails)}), 200
    except Exception as e:
...
def read_one_admin(trail_id):
    try:
        trail = db.session.execute(
            text("EXEC CW2.ReadTrailAdministrator @TrailID = :TrailID"),
            {"TrailID": trail_id}
        ).fetchone()
        if not trail:
...
def read_one_standard(trail_id):
    try:
        trail = db.session.execute(
            text("EXEC CW2.ReadTrailStandard @TrailID = :TrailID"),
            {"TrailID": trail_id}
        ).fetchone()
        if not trail:
            return jsonify({"error": f"Trail with ID {trail_id} not found"}), 404
